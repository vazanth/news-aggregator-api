...
const { fetchAllNews, fetchTopHeadlines } = require('../../src/services/newsService');
...
describe('Verify if the data from external api is set in cache', () => {
  ...
  it('should store the external api data to cache for logged-in users', async () => {
    cacheManager.keys.mockReturnValue(['123-loggedIn', 'fileIo', '234-loggedIn']);
    ...
    cacheManager.set.mockReturnValue(`123-loggedIn-AllNews`, JSON.stringify(dummyNewsData));
    cacheManager.set.mockReturnValue(`123-loggedIn-TopNews`, JSON.stringify(dummyNewsData));
    ...
    expect(logger.info).toHaveBeenCalledWith('News cache updated.');
  });
  ...
  it('should not cache any data and log the appropriate message to console', async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith('No active users to update cache.');
  });
});
